...
| **P-03** | Data residency / in-country | Applies | Region pinned `us-central1` everywhere; Terraform validates it and fails fast (`variables.tf`); regional CMEK (`kms.tf`); VPC-SC perimeter (`vpc_sc.tf`); BigQuery + GCS pinned in-region. |
