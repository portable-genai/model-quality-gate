"""Remote-platform knowledge-base adapter : thin HTTP client to enterprise-knowledge-base.

For grounded evaluation the gate retrieves the reference context a golden input should have
been answered from. That context comes from
...
This adapter implements :class:`KnowledgeBaseClientPort` by POSTing the query and mapping each
returned passage to a domain :class:`Citation`. The base URL is read from ``KNOWLEDGE_BASE_URL``
with a localhost default. The gate evaluates models, not customer data, so no PII crosses this
boundary.
"""
...
class RemoteKnowledgeBaseError(AiQualityError):
    """Raised when the remote enterprise-knowledge-base service returns a non-2xx response."""


class RemoteKnowledgeBaseAdapter:
    """HTTP client for the ``enterprise-knowledge-base`` search endpoint."""
    ...
    def retrieve(self, query: str, top_k: int = 5) -> list[Citation]:
        """Return reference citations for ``query`` from enterprise-knowledge-base."""
        ...
        try:
            ...
            raise RemoteKnowledgeBaseError(
                f"enterprise-knowledge-base search request to {url} failed: {exc}"
            ) from exc
        if response.status_code // 100 != 2:
            raise RemoteKnowledgeBaseError(
                f"enterprise-knowledge-base search {url} returned {response.status_code}: "
                f"{response.text[:500]}"
            )
