# SPEC : `model-quality-gate` AI Quality & Model-Risk Platform
...
The **production promotion gate** and model-risk system for the catalog. It is an
...
(dependency rule R5). It returns a PASS/FAIL gate verdict and the evidence behind it.

It does **not** process customer PII: it evaluates models against datasets, not customer
data. Dependency rule R1 / `agent-guardrail-gateway` (customer-PII guardrail) is therefore **N/A** here (see
