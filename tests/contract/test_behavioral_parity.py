...
``model-quality-gate`` is the eval / model-risk promotion AUTHORITY, not a customer-facing
agent: it evaluates models against datasets and processes no customer PII. It consumes two
platform siblings over real HTTP clients (``config/settings.yaml``): the
**``enterprise-knowledge-base``** for grounded reference retrieval and the
**``agent-observability``** service for the WORM audit sink. Those two ports have a real
``platform`` implementation, so for each we put the SAME request through ``local`` and
``platform`` and require identical domain-level behavior:

* ``knowledge_base`` (KnowledgeBaseClientPort -> ``enterprise-knowledge-base``): the local
  FTS5 index and the ``platform`` httpx client (respx-mocked at the documented
  ``/v1/search`` contract) return the SAME first-class :class:`Citation` objects for one
  query; a local re-run over a fresh in-memory index is identical (determinism);
  ``onprem`` raises.
* ``audit`` (AuditSinkPort -> ``agent-observability``): the local hash-chained store reads
  back exactly the serialized :class:`AuditEvent`, and the ``platform`` client POSTs a
  byte-identical body to ``/v1/audit`` (respx-mocked); ``onprem`` raises.
...
def _passage_json(citation: Citation) -> dict[str, Any]:
    """Render a :class:`Citation` into the ``enterprise-knowledge-base``
    ``/v1/search`` passage shape (SPEC §6).
...
# --------------------------------------------------------------------------- #
# KnowledgeBaseClientPort (``enterprise-knowledge-base``) : identical citations whether local
# or platform
# --------------------------------------------------------------------------- #
def test_knowledge_base_parity_same_citations_across_implementations():
    ...
    with respx.mock:
        # ``enterprise-knowledge-base`` serves the SAME passages for the same query at the
        # documented /v1/search shape.
...
def test_audit_parity_identical_payload_at_every_sink():
    """The local WORM store and the ``agent-observability`` client see byte-identical audit
    payloads."""
    event = AuditEvent(
        action="gate",
    ...
    # platform sink (``agent-observability``): the POSTed body is byte-identical to what
    # local stored.
    with respx.mock:
...
def test_evaluation_scorer_is_deterministic_and_onprem_fails_fast():
    ...
    ``gcp`` needs the Google Cloud SDK and there is no ``platform`` evaluator
    (``model-quality-gate`` IS the eval authority, it does not call a sibling to evaluate),
    so behavioral parity for this
