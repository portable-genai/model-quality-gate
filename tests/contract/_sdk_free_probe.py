...
``tests/contract`` carries no ``__init__.py`` and needs none: ``tests`` is a package, so the
subdirectory resolves as a namespace portion within it and the ``-m`` invocation above works
as written. Adding a marker file here would also reclassify ``tests`` for the import sorter
and churn unrelated suites.
"""
