...
``model-quality-gate`` IS the promotion authority, so its own offline gate validates the gate
LOGIC rather than a model's answers: each golden scenario carries the eval scores a backend
would produce and the verdict the gate should reach. That makes falsification more important
here, not less. If these four metrics cannot fail, the service that decides whether every
other system may promote is itself unchecked.
