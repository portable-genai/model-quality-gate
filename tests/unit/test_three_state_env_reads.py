...
TWO_STATE_READS_WITH_A_REASON: dict[str, str] = {
    "DEMO_SELFTEST_OUT": (
        "This path only selects where the offline demo self-test writes its synthetic "
        "transcript; it names no service, credential, model, dataset or access boundary."
    ),
    "PORT": (
    ...
    "PORTABILITY_OUT": (
        "This path only selects where the SDK-free portability walkthrough writes its synthetic "
        "JSON evidence; it reaches no service and grants no capability."
    ),
    "SLOWMO_MS": (
        "This value only changes Playwright animation delay for a presenter. Invalid or empty "
        "values fail int() visibly and cannot relax an application control."
    ),
}
