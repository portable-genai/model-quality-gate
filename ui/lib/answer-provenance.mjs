...
// a same-origin path such as `/agent/api` when mounted), so the service's responses must carry
// `Access-Control-Expose-Headers` naming both headers: a cross-origin response hides every header
// not listed there, and the pills would stay on the configured model forever with every test
// green. The kit's middleware sends that list itself, and `tests/unit/test_answer_provenance.py`
// proves it on a cross-origin request.
//
