...
def build_spec() -> PluginSpec:
    ...
    from model_quality_gate.adapters.gcp.mcp_tool_catalog import McpToolCatalogAdapter
    from model_quality_gate.agent.agent_card import agent_card_document
    ...
    return PluginSpec(
        ...
        license="Apache-2.0",
        repository="https://github.com/portable-genai/model-quality-gate",
        ...
        servers={
            "model-quality-gate": StdioServer(
                command="python",
                args=("-m", "model_quality_gate.mcp"),
                cwd="${PLUGIN_ROOT}",
            )
        },
